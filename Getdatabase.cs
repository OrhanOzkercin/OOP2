...
namespace Oop2
{
    class Getdatabase
    {
        ...
        public void IsAdmin(Costumer costumer, ref bool flag)
        {
            try
            {
                dh.baglanti.Open();
                SqlCommand komut = new SqlCommand("Select * from Tbl_Uyeler where Username=@Username", baglanti);
                komut.Parameters.AddWithValue("Username", costumer.Username);
                SqlDataReader dr = komut.ExecuteReader();
                dr.Read();
                if (costumer.Password == (string)dr["Password"] && (bool)dr["Admincheck"] == true)
                {
                    flag = true;
                }
                else
